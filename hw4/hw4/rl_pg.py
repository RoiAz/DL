...
class PolicyNet(nn.Module):
    def __init__(self, in_features: int, out_actions: int, **kw):
        ...
        layers.append(nn.Linear(in_feat_num, out_actions))
        self.net = nn.Sequential(*layers)
        self.net.to(kw.get('device', 'cpu')) #check if we get gpu if not run it on cpu
        # ========================
...
class PolicyAgent(object):
    ...
    def current_action_distribution(self) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            net_out = self.p_net(curr)
            probs = torch.softmax(net_out, dim=-1)
    ...
    def step(self) -> Experience:
        ...
        # ====== YOUR CODE: ======
        act = torch.multinomial(self.current_action_distribution(), 1).item()
        observation, reward, is_done, _ = self.env.step(act)
        experience = Experience(self.curr_state, act, reward, is_done)
        self.curr_state = torch.from_numpy(observation)
...
class VanillaPolicyGradientLoss(nn.Module):
    ...
    def _policy_loss(self, batch, action_scores, policy_weight):
        ...
        # ====== YOUR CODE: ======
        probs = torch.log_softmax(action_scores, dim=-1)
        pie = torch.squeeze(probs.gather(1, torch.unsqueeze(batch.actions, 1)))
        llist = policy_weight * pie
        loss_p = - torch.mean(llist)
        # ========================
        return loss_p
...
class BaselinePolicyGradientLoss(VanillaPolicyGradientLoss):
    def forward(self, batch: TrainBatch, action_scores: torch.Tensor, **kw):
        ...
        vanq = policy_weight - baseline
        loss_p = self._policy_loss(batch, action_scores, vanq)
        # ========================
