...
class Generator(nn.Module):
    def __init__(self, z_dim, featuremap_size=4, out_channels=3):
        ...
        stride = 2
        padding = 1
        channels = [512, 256, 256, 128, out_channels]

        module = []
        # middle layers
        for i in range(len(channels) - 1):
            module.append(nn.ConvTranspose2d(in_channels=channels[i], out_channels=channels[i + 1], kernel_size=kernel_size, stride=stride, padding= padding, bias=False))
            # for evey layyer exept the last one
            if i < len(channels) - 2:
        ...
        # save the model
        self.extract = nn.Sequential(*module)
        self.sizes_to_view = (channels[0],featuremap_size,featuremap_size)
        
        self.pre_linear= nn.Linear(z_dim, channels[0]*(featuremap_size * featuremap_size))
        # ========================
    ...
    def sample(self, n, with_grad=False):
        ...
        # forward
        rand = torch.randn(n, self.z_dim, requires_grad=with_grad).to(device)
        samples = self.forward(rand)
    ...
    def forward(self, z):
        ...
        # ====== YOUR CODE: ======
        tmp = self.pre_linear(z)
        reshape = tmp.view([z.shape[0],-1,self.sizes_to_view[1],self.sizes_to_view[2]])
        x = self.extract(reshape)
        # ========================
        return x
...
def train_batch(
    ...
    # gene forward
    gene_pre = dsc_model(gene_data)

    # get gene loss
    gen_loss = gen_loss_fn(gene_pre)
    gen_loss.backward()
    gen_optimizer.step()
    # ========================
